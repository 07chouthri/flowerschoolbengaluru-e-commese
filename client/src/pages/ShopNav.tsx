...
export default function ShopNav() {
  ...
  const [searchSuggestions, setSearchSuggestions] = useState<Array<{item: string, category: string, id?: string}>>([]);
  ...
  // Optional: Speech synthesis for animated text
  ...
  // Optimized search handlers with live API suggestions
  const handleSearchChange = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (value.trim()) {
      ...
      try {
        ...
        // Only fetch a few for suggestions
        params.set('limit', '6');
        const response = await fetch(`/api/products?${params.toString()}`, { credentials: 'include' });
        if (response.ok) {
          const data = await response.json();
          // Show product names as suggestions
          const suggestions = Array.isArray(data)
            ? data.slice(0, 6).map((prod: any) => ({ item: prod.name, category: prod.category || 'Product', id: prod.id }))
            : [];
  ...
  const handleSuggestionClick = useCallback((suggestion: {item: string, category: string, id?: string}) => {
    setSearchQuery("");
    setShowSuggestions(false);
    if (suggestion.id) {
  ...
  // Slow, smooth typing animation with fade-in and optional speech
  const animateTyping = useCallback(() => {
    if (searchQuery) return;
    const now = Date.now();
    // Throttle updates to ~10fps (very slow and smooth)
    ...
    if (isDeleting) {
      ...
      setAnimatedText(prev => {
        ...
        if (newText === currentCategory) {
          setTimeout(() => setIsDeleting(true), 1800); // Even longer pause before deleting
        }
        return newText;
      });
    }
  ...
  // Start typing animation and blinking cursor
  useEffect(() => {
    if (!searchQuery) {
      rafRef.current = requestAnimationFrame(animateTyping);
    }
    const cursorInterval = setInterval(() => setShowCursor(c => !c), 600);
    return () => {
      if (rafRef.current) {
  ...
  const handleInputFocus = useCallback(() => {
    // Stop animation and clear animatedText on focus
    if (rafRef.current) {
      cancelAnimationFrame(rafRef.current);
    }
    setAnimatedText("");
            ...
            <div className="pb-3" ref={searchRef}>
              <div className="relative">
                ...
                <div className="absolute inset-y-0 left-4 flex items-center pointer-events-none max-w-[60%]">
                  <span
                    className={`text-gray-500 font-medium text-xs truncate transition-opacity duration-700 ${fadeIn ? 'opacity-100' : 'opacity-0'}`}
                    style={{letterSpacing: '0.5px'}}
                  >
                    {!searchQuery && animatedText ? `Search for ${animatedText}` : ""}
                    {!searchQuery && animatedText && showCursor && <span className="font-bold" style={{animation: 'none'}}>|</span>}
                  </span>
                </div>
            ...
            <div className="flex-1 max-w-xl mx-4 md:mx-6" ref={searchRef}>
              <div className="relative">
                ...
                <div className="absolute inset-y-0 left-4 flex items-center pointer-events-none w-3/4">
                  <span
                    className={`text-gray-500 font-medium text-xs md:text-sm truncate transition-opacity duration-700 ${fadeIn ? 'opacity-100' : 'opacity-0'}`}
                    style={{letterSpacing: '0.5px'}}
                  >
                    {!searchQuery && animatedText ? `Search for ${animatedText}` : ""}
                    {!searchQuery && animatedText && showCursor && <span className="font-bold" style={{animation: 'none'}}>|</span>}
