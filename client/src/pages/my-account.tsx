  ...
  return (
    <div className="space-y-3 sm:space-y-4 max-h-[80vh] overflow-y-auto px-1">
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-3 sm:space-y-4">
        ...
        <div>
          <Label htmlFor="email" className="text-sm">Email Address * (must end with .com)</Label>
          <Input
            id="email"
            type="email"
            {...form.register("email")}
...
export default function MyAccount() {
  ...
  const { data: orders = [], isLoading: ordersLoading, error: ordersError } = useQuery<Order[]>({
    ...
    queryFn: async () => {
      ...
      try {
        // Pass user ID as a query param or in the body as required by your backend
        const response = await apiRequest(`/api/orders/user?userId=${encodeURIComponent(profile.id)}`);
        if (!response.ok) {
          throw new Error(`Failed to fetch orders: ${response.status}`);
        }
        const data = await response.json();
        // Ensure the result is always an array of order objects
        if (Array.isArray(data)) {
          return data;
        } else if (data && typeof data === 'object' && data.orders && Array.isArray(data.orders)) {
          return data.orders;
        } else {
          return [];
        }
